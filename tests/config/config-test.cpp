...
namespace {
    ...
    public:
        ...
        std::string write_config(const nlohmann::json &content) const {
            return write_config(content.dump(2));
        }
    ...
    TEST_CASE("empty_file_returns_defaults") {
        ConfigFileHarness harness;
        const auto path = harness.write_config(std::string{});

        const auto cfg = Config::load_from(path);
        CHECK(cfg.model == "claude-sonnet-4-20250514");
    };

    TEST_CASE("invalid_json_returns_defaults") {
        ConfigFileHarness harness;
        const auto path = harness.write_config(std::string{"this is [not valid json"});
    ...
    TEST_CASE("missing_file_returns_defaults") {
        const auto cfg = Config::load_from("/tmp/orangutan_nonexistent_config_xyz.json");
        CHECK(cfg.model == "claude-sonnet-4-20250514");
    };
    ...
    TEST_CASE("loads_protected_legacy_agent_api_key_with_explicit_password") {
        ...
        nlohmann::json doc = {
            {"agent", {{"api_key", protected_key}}},
        };
        const auto path = harness.write_config(doc);
    ...
    TEST_CASE("loads_protected_configured_agent_api_key_from_environment_password") {
        ...
        nlohmann::json doc = {
            {"agents", {{"default", {{"api_key", protected_key}}}}},
        };
        const auto path = harness.write_config(doc);
    ...
    TEST_CASE("named_agents_can_inherit_protected_legacy_api_key") {
        ...
        nlohmann::json doc = {
            {"agent", {{"api_key", protected_key}, {"model", "legacy-model"}}},
            {"agents", {{"coder", {{"model", "coder-model"}}}}},
        };
        const auto path = harness.write_config(doc);
    ...
    TEST_CASE("loads_protected_qq_client_secret_with_explicit_password") {
        ...
        nlohmann::json doc = {
            {"agents", {{"default", {{"model", "test-model"}, {"api_key", "test-key"}}}}},
            {"qq", {{"app_id", "qq-app"}, {"client_secret", protected_secret}}},
        };
        const auto path = harness.write_config(doc);
