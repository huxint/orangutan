...
public:
    ...
    json start_background_shell(json input) {
        ...
        boost::ut::expect(not result.is_error) << result.content;
        if (result.is_error) {
            return {};
        }
    ...
    std::vector<orangutan::automation::InboxItem> wait_for_inbox_size(size_t expected_size, std::chrono::milliseconds timeout = std::chrono::seconds(5)) {
        ...
        boost::ut::expect(false) << "background completion inbox items did not arrive in time";
    ...
    std::vector<std::string> wait_for_resume_messages_size(size_t expected_size, std::chrono::milliseconds timeout = std::chrono::seconds(5)) {
        ...
        boost::ut::expect(false) << "background completion resume messages did not arrive in time";
        std::scoped_lock lock(resume_state_->mutex);
        return resume_state_->messages;
    }
...
boost::ut::suite background_shell_completion_suite = [] {
    ...
    "shell_tool_schema_documents_on_complete_policy"_test = [] {
        ...
        expect(shell != nullptr) << "shell tool definition missing";
        if (shell == nullptr) {
            return;
        }
        ...
        expect(on_complete.at("type") == "object");
        expect(on_complete.contains("properties"));
        expect(on_complete["properties"].contains("mode"));
        expect(on_complete["properties"].contains("prompt"));
        expect(on_complete["properties"]["mode"]["enum"] == json::array({"inbox", "resume"}));
    };

    "inbox_only_completion_routing_exposes_on_complete_but_rejects_resume"_test = [] {
        BackgroundShellCompletionHarness harness;
        ...
        expect(shell->input_schema.contains("properties"));
        expect(shell->input_schema["properties"].contains("on_complete"));
        expect(shell->input_schema["properties"]["on_complete"]["properties"]["mode"]["enum"] == json::array({"inbox"}));
        ...
        expect(result.content.find("resume") != std::string::npos);
        expect(result.content.find("not available") != std::string::npos);
    };

    "background_shell_defaults_to_inbox_only_completion_routing"_test = [] {
        BackgroundShellCompletionHarness harness;
        ...
        expect(items.size() == 1U);
        {
            ...
            expect(harness.resume_state_->messages.empty());
        }
        if (items.size() != 1U) {
            return;
        }
        ...
        expect(completion.at("type") == "background_process_completion");
        expect(completion.at("process_id") == start_payload.at("process_id"));
        expect(completion.at("on_complete").at("mode") == "inbox");
    };

    "resume_completion_preserves_prompt_in_inbox_and_resume_payload"_test = [] {
        BackgroundShellCompletionHarness harness;
        ...
        expect(resume_messages.size() == 1U);
        ...
        expect(inbox_completion.at("process_id") == start_payload.at("process_id"));
        expect(inbox_completion.at("on_complete").at("mode") == "resume");
        expect(inbox_completion.at("on_complete").at("prompt") == prompt);
        expect(resume_completion.at("process_id") == start_payload.at("process_id"));
        expect(resume_completion.at("on_complete").at("mode") == "resume");
        expect(resume_completion.at("on_complete").at("prompt") == prompt);
    };

    "large_resume_prompt_is_bounded_in_persisted_and_resume_payload"_test = [] {
        BackgroundShellCompletionHarness harness;
        ...
        expect(inbox_completion.at("process_id") == start_payload.at("process_id"));
        expect(inbox_prompt == resume_prompt);
        expect(inbox_prompt.size() < prompt.size());
        expect(inbox_prompt.size() <= background_completion_prompt_max_chars);
        expect(inbox_prompt.ends_with("..."));
        expect(inbox_prompt.find(prompt_unit) != std::string::npos);
        expect(items.front().body.size() <= background_completion_payload_max_bytes);
        expect(resume_messages.front().size() <= background_completion_payload_max_bytes);
    };

    "unsupported_completion_routing_does_not_advertise_or_accept_on_complete"_test = [] {
        BackgroundShellCompletionHarness harness;
        ...
        expect(result.is_error);
        expect(result.content.find("on_complete") != std::string::npos);
        expect(result.content.find("not available") != std::string::npos);
    };

    "completion_payload_summaries_are_scrubbed_before_persistence"_test = [] {
        BackgroundShellCompletionHarness harness;
        ...
        expect(inbox_payload.at("stdout").at("tail").get<std::string>().find("[REDACTED]") != std::string::npos);
        expect(inbox_payload.at("stderr").at("tail").get<std::string>().find("[REDACTED]") != std::string::npos);
        expect(inbox_payload.at("stdout").at("tail").get<std::string>().find(secret) == std::string::npos);
        expect(inbox_payload.at("stderr").at("tail").get<std::string>().find(secret) == std::string::npos);
        expect(resume_payload.at("stdout").at("tail").get<std::string>().find("[REDACTED]") != std::string::npos);
        expect(resume_payload.at("stderr").at("tail").get<std::string>().find("[REDACTED]") != std::string::npos);
        expect(resume_payload.at("stdout").at("tail").get<std::string>().find(secret) == std::string::npos);
        expect(resume_payload.at("stderr").at("tail").get<std::string>().find(secret) == std::string::npos);
    };

    "completion_titles_are_scrubbed_before_persistence"_test = [] {
        BackgroundShellCompletionHarness harness;
        ...
        expect(items.front().title.find(secret) == std::string::npos);
        expect(items.front().title.find("[REDACTED]") != std::string::npos);
    };

    "invalid_utf8_output_and_prompt_still_produce_completion_payload"_test = [] {
        BackgroundShellCompletionHarness harness;
        ...
        expect(inbox_payload.at("on_complete").at("prompt") == expected_prompt);
        expect(resume_payload.at("on_complete").at("prompt") == expected_prompt);
        expect(inbox_payload.at("stdout").at("tail") == expected_stdout);
        expect(resume_payload.at("stdout").at("tail") == expected_stdout);
        expect(inbox_payload.at("stderr").at("tail") == expected_stderr);
        expect(resume_payload.at("stderr").at("tail") == expected_stderr);
    };

    "completion_resume_runs_through_agent_execution_lease"_test = [] {
        BackgroundShellCompletionHarness harness;
        ...
        ScriptedProvider provider({
            [&provider_started_promise](const std::vector<Message> &messages) {
                ...
                boost::ut::expect(not messages.empty());
                if (messages.empty()) {
                    return LLMResponse{};
                }
  ...
  "process_id": "proc-lease"
            ...
            expect(provider_started.wait_for(std::chrono::milliseconds(150)) == std::future_status::timeout);
        }
    ...
    "resume_dispatch_failures_become_inbox_visible_notes"_test = [] {
        ...
        expect(completion_item != nullptr) << "completion inbox item missing";
        expect(failure_item != nullptr) << "failure inbox item missing";
        ...
        expect(completion.at("process_id") == "proc-test");
        expect(completion.at("on_complete").at("mode") == "resume");
        expect(failure.at("process_id") == "proc-test");
        expect(failure.at("reason") == "resume callback failed");
        expect(completion_item->title.find(secret) == std::string::npos);
        expect(failure_item->title.find(secret) == std::string::npos);
        expect(completion_item->title.find("[REDACTED]") != std::string::npos);
        expect(failure_item->title.find("[REDACTED]") != std::string::npos);
    };
};
