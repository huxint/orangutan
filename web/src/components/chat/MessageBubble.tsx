      ...
      components={{
        p({ children, node, ...rest }) {
          ...
          if (hasBlockChild(children)) {
            return (
              <div
                className="my-2.5 leading-[1.75] first:mt-0 last:mb-0"
                {...rest}
              >
                {children}
              </div>
          ...
          return (
            <p className="my-2.5 leading-[1.75] first:mt-0 last:mb-0" {...rest}>
              {children}
            </p>
          );
        },
        h1({ children }) {
          return (
            <h1 className="mt-6 mb-3 text-lg font-bold first:mt-0">
              {children}
            </h1>
          );
        },
        h2({ children }) {
          return (
            <h2 className="mt-5 mb-2.5 text-base font-bold first:mt-0">
              {children}
            </h2>
          );
        },
        h3({ children }) {
          return (
            <h3 className="mt-4 mb-2 text-sm font-bold first:mt-0">
              {children}
            </h3>
          );
        },
        ul({ children }) {
          return (
            <ul className="my-2.5 list-disc space-y-1 pl-5 marker:text-text-muted">
              {children}
            </ul>
          );
        },
        ol({ children }) {
          return (
            <ol className="my-2.5 list-decimal space-y-1 pl-5 marker:text-text-muted">
              {children}
            </ol>
          );
        },
...
export function MessageBubble({ role, content }: MessageBubbleProps) {
  if (role === "user") {
    ...
    return (
      <div className="flex justify-end">
        <div
          className="max-w-[75%] rounded-2xl rounded-br-sm px-4 py-2.5 text-[14.5px]
  ...
  return (
    <div className="max-w-[85%]">
      <div
        className="space-y-2 rounded-2xl rounded-bl-sm
