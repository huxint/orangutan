...
function appendAssistantBlock(
  ...
  if (block.type === "tool_result" && block.tool_use_id) {
    const existingIndex = content.findIndex(
      (item) =>
        item.type === "tool_result" && item.tool_use_id === block.tool_use_id,
    );
    if (existingIndex >= 0) {
      const next = [...content];
      next[existingIndex] = block;
...
export function ChatView() {
  const { agentKey: paramAgentKey, sessionId: paramSessionId } = useParams<{
    agentKey: string;
    sessionId?: string;
  }>();
  ...
  const normalizeMessages = useCallback(
    (sessionMessages: SessionMessage[]): ChatMessage[] => {
      ...
      for (const message of sessionMessages) {
        if (message.role === "assistant") {
          ...
          normalized[normalized.length - 1] = {
            ...normalized[normalized.length - 1],
            content: message.content.reduce(
        ...
        const userTextBlocks = message.content.filter(
        ...
        if (toolResultBlocks.length > 0) {
          ...
          normalized[normalized.length - 1] = {
            ...normalized[normalized.length - 1],
            content: toolResultBlocks.reduce(
              appendAssistantBlock,
              normalized[normalized.length - 1].content,
            ),
          };
  ...
  const [sessionMeta, setSessionMeta] = useState<ChatSessionSummary | null>(
    null,
  );
  ...
  const transientAssistantMessageRef = useRef<{
    sessionId: string;
    message: ChatMessage;
  } | null>(null);
  ...
  const loadSession = useCallback(
    (targetAgentKey: string, targetSessionId: string) => {
  ...
  useEffect(() => {
    ...
    getAgents()
      .then((agents) => {
        ...
        const selectedAgent =
          agents.find((candidate) => candidate.key === agentKey) ?? null;
        if (!selectedAgent) {
          setAgent(null);
          setAgentSessions([]);
  ...
  useEffect(() => {
    ...
    loadSession(agentKey, paramSessionId).catch((loadError) => {
      if (cancelled) return;
      setSessionMeta(null);
      setMessages([]);
      setError(
  ...
  const handleSend = useCallback(
    (text: string) => {
      ...
      const finalizeRequest = () => {
        const switchedToDifferentSession =
          Boolean(activeSessionId) && paramSessionId !== activeSessionId;
          ...
          switch (type) {
            case "session": {
              ...
              if (paramSessionId !== sessionData.session_id) {
                ...
                navigate(`/chat/${agentKey}/${sessionData.session_id}`, {
                  replace: requestWasForNewSession,
                });
              }
              break;
            }
  ...
  const sessionLoading =
    Boolean(paramSessionId) && sessionMeta?.id !== paramSessionId;
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <MessageList
        messages={messages}
        onSuggest={!readOnly && !streaming ? handleSuggest : undefined}
      />
      {pendingApproval && (
        <ApprovalPrompt
