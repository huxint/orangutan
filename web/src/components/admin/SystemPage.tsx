    ...
    <div className="p-6 h-full overflow-y-auto pb-20">
      ...
      {error ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            { icon: Clock, label: 'Uptime', value: formatUptime(status.uptime_seconds), delay: 0 },
            { icon: Users, label: 'Sessions', value: String(status.active_web_sessions), delay: 0.05 },
          ].map(card => (
            <motion.div
              key={card.label}
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <div className="p-1.5 rounded-lg bg-accent-dim text-accent"><card.icon size={14} /></div>
                <span className="text-[10px] font-semibold uppercase tracking-widest text-text-muted">{card.label}</span>
              </div>
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              <div className="p-1.5 rounded-lg bg-success-dim text-success"><Activity size={14} /></div>
