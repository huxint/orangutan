...
interface Tool { name: string; description: string; source: string }

// Map well-known tool names to icons and accent colors
const TOOL_META: Record<string, { icon: LucideIcon; color: string; bg: string }> = {
...
const DEFAULT_META = { icon: Wrench, color: 'text-accent', bg: 'bg-accent-dim' }
...
export function ToolsPage() {
  ...
  const load = () => {
    ...
    apiFetch<Tool[]>('/api/tools').then(setTools).catch(e => setError(e.message)).finally(() => setLoading(false))
  }
    ...
    <div className="p-6 h-full overflow-y-auto pb-10">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          {!loading && !error && (
            <p className="text-xs text-text-muted mt-0.5">{tools.length} tools registered across {new Set(tools.map(t => t.source)).size} sources</p>
          )}
        </div>
      </div>
            ...
            {Array.from(groups.entries()).map(([source, sourceTools]) => (
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-text-muted">{source}</span>
                  <div className="flex-1 h-px bg-border" />
                  <span className="text-[10px] text-text-muted">{sourceTools.length}</span>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div className={cn('shrink-0 p-2 rounded-lg', meta.bg)}>
                            <Icon size={16} className={meta.color} />
                          </div>
                          <div className="min-w-0 flex-1">
                            <code className={cn('font-mono text-sm font-bold', meta.color)}>{t.name}</code>
