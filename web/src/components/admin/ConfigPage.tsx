  ...
  return (
    <div
      className={cn(
        ...
        checked
          ? "border-accent/25 bg-accent-dim"
...
export function ConfigPage() {
  ...
  const setMem = (key: keyof Memory, val: unknown) =>
    setConfig({ ...config, memory: { ...config.memory, [key]: val } });
  ...
  return (
    <div className="p-6 max-w-2xl space-y-3 overflow-y-auto h-full pb-10">
      ...
      <Section icon={Bot} title="Agent" color="bg-sky-500/10 text-sky-400">
        ...
        <Field label="Base URL" hint="API endpoint">
          <input
            className={inputCls}
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <Field label="Max Iterations">
            <input
              ...
              onChange={(e) =>
                set("max_iterations", parseInt(e.target.value) || 0)
              }
            />
          </Field>
        ...
        <Field label="Fallback Models" hint="comma-separated">
          <input
            className={inputCls}
            value={config.fallback_models.join(", ")}
            onChange={(e) =>
              set(
                "fallback_models",
                e.target.value
      ...
      >
        <Field label="Edit Mode">
          <input
            className={inputCls}
            value={config.edit_mode}
            onChange={(e) => set("edit_mode", e.target.value)}
          />
        </Field>
        ...
        <Field label="Denied Tools" hint="comma-separated">
          <input
            className={inputCls}
            value={config.denied_tools.join(", ")}
            onChange={(e) =>
              set(
                "denied_tools",
                e.target.value
      ...
      >
        <Toggle
          ...
          label="Auto Save"
          description="Automatically persist sessions to disk"
        />
      </Section>
      ...
      >
        <Toggle
          checked={config.memory.mirror_enabled}
          onChange={(v) => setMem("mirror_enabled", v)}
          label="Mirror Enabled"
          description="Mirror conversation to a file"
        />
        <Field label="Mirror File">
          <input
            className={inputCls}
            value={config.memory.mirror_file}
            onChange={(e) => setMem("mirror_file", e.target.value)}
          />
        </Field>
        <Field label="Journal Dir">
          <input
            className={inputCls}
            value={config.memory.journal_dir}
            onChange={(e) => setMem("journal_dir", e.target.value)}
          />
        </Field>
      </Section>
    </div>
